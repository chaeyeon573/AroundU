...
/** 공개 범위 라디오 목록 (웹 VisibilityList 와 동일) */
...
/** 공개 범위 선택 — compact 모드는 한 줄 버튼 + 시트, 기본은 라디오 리스트 */
export function VisibilityPicker({ value, onChange, options = VISIBILITY_ORDER, compact, label = t('공개 범위') }: { value: Visibility; onChange: (v: Visibility) => void; options?: Visibility[]; compact?: boolean; label?: string }) {
  const [open, setOpen] = useState(false);
  const Icon = VISIBILITY_ICONS[value];
  if (compact) {
    return (
      <>
        <Pressable onPress={() => setOpen(true)} style={tw`h-8 px-2.5 rounded-full bg-surface-2 flex-row items-center`}>
          <Icon size={13} color={C.ink2} /><Text style={tw`mx-1 text-[12px] font-semibold text-ink-2`}>{VISIBILITY_LABELS[value]}</Text><ChevronRight size={12} color={C.ink3} />
        </Pressable>
