...
export function ChatInboxPage() {
  ...
  return (
    <div className="min-h-full">
      <TopBar back title="메시지" bell />
      <div className="px-4 pt-2">
        <Segmented value={tab} onChange={(t) => setParams({ tab: t })} options={[{ value: 'direct', label: '개인' }, { value: 'activity', label: '활동' }, { value: 'org', label: '조직' }, { value: 'requests', label: reqCount ? `요청 ${reqCount}` : '요청' }]} />
      </div>
        ...
        <div className="px-4 py-3">
          {list.length === 0 ? (
            <EmptyState emoji={tab === 'direct' ? '💬' : tab === 'activity' ? '🗓️' : '🏛️'}
              title={tab === 'direct' ? '아직 개인 채팅이 없어요' : tab === 'activity' ? '참가 중인 활동 채팅이 없어요' : '조직 채팅이 없어요'}
              description={tab === 'direct' ? '친구 요청 수락, 상호 관심, 활동 참가 승인 중 하나가 성립하면 대화를 시작할 수 있어요.' : tab === 'activity' ? '활동에 참가하면 그룹 채팅방이 열려요.' : '동아리·조직에 가입하거나 팔로우해보세요.'}
              action={<Button variant="outline" onClick={() => nav(tab === 'direct' ? '/' : tab === 'activity' ? '/map' : '/community')}>둘러보기</Button>} />
          ) : (
                ...
                return (
                  <button key={r.id} onClick={() => nav(`/chats/${r.id}`)} className="w-full flex items-center gap-3 px-3.5 py-3 text-left press">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={cn('text-[13px] truncate mt-0.5', n > 0 ? 'text-ink font-medium' : 'text-ink-3')}>{lm?.text ?? '대화를 시작해보세요'}</div>
                    </div>
      ...
      {status === 'ready' && tab === 'requests' && (
        <div className="px-4 py-3 space-y-4">
          <section>
            <h2 className="text-[14px] font-bold mb-2 flex items-center gap-1.5"><Inbox size={15} />받은 활동 제안 {inProposals.length}</h2>
            {inProposals.length === 0 ? <div className="card p-4 text-[13px] text-ink-3">받은 제안이 없어요.</div> : inProposals.map((p) => { const u = v.userById(p.fromId); return (
              <div key={p.id} className="card p-3.5 mb-2">
                ...
                <div className="flex gap-2 mt-3"><Button full variant="outline" icon={<X size={15} />} onClick={() => run(() => api.proposals.respond(p.id, false))}>이번에는 어려워요</Button><Button full icon={<Check size={15} />} onClick={async () => { await run(() => api.proposals.respond(p.id, true), '제안을 수락했어요. 대화를 시작해요!'); setParams({ tab: 'direct' }); }}>좋아요</Button></div>
              </div>
            ); })}
          </section>
          <section>
            <h2 className="text-[14px] font-bold mb-2 flex items-center gap-1.5"><Users size={15} />받은 친구 요청 {inRequests.length}</h2>
            {inRequests.length === 0 ? <div className="card p-4 text-[13px] text-ink-3">받은 친구 요청이 없어요.</div> : inRequests.map((q) => { const u = v.userById(q.fromId); return (
              <div key={q.id} className="card p-3.5 mb-2 flex items-center gap-3">
                ...
                <Button size="sm" variant="outline" onClick={() => run(() => api.relationships.respondFriendRequest(q.id, false))}>거절</Button>
                <Button size="sm" onClick={() => run(() => api.relationships.respondFriendRequest(q.id, true), `${u?.nickname}님과 친구가 되었어요!`)}>수락</Button>
              </div>
            ); })}
            <p className="text-[11px] text-ink-3 mt-1">거절해도 상대에게 알림이 가지 않아요.</p>
          </section>
          {(outProposals.length > 0 || outRequests.length > 0) && (
            <section>
              <h2 className="text-[14px] font-bold mb-2 flex items-center gap-1.5"><MessageCircle size={15} />보낸 요청</h2>
              <div className="card divide-y divide-line">
                {outRequests.map((q) => { const u = v.userById(q.toId); return <div key={q.id} className="flex items-center gap-3 px-3.5 py-3"><Avatar emoji={u?.avatar.emoji ?? '👤'} hue={u?.avatar.hue ?? 200} size={36} /><div className="flex-1 text-[13px]"><b>{u?.nickname}</b>님에게 친구 요청</div><Tag>대기 중</Tag></div>; })}
                {outProposals.map((p) => { const u = v.userById(p.toId); return <div key={p.id} className="flex items-center gap-3 px-3.5 py-3"><Avatar emoji={u?.avatar.emoji ?? '👤'} hue={u?.avatar.hue ?? 200} size={36} /><div className="flex-1 text-[13px]"><b>{u?.nickname}</b>님에게 {CATEGORY_LABELS[p.category]} 제안</div>{p.status === 'pending' ? <Tag>대기 중</Tag> : <Tag tone="neutral">이번 활동은 성사되지 않았어요</Tag>}</div>; })}
              </div>
              <p className="text-[11px] text-ink-3 mt-1">읽음 여부와 거절 사유는 표시되지 않아요.</p>
            </section>
          )}
        </div>
      )}
