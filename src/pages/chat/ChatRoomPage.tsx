...
export function ChatRoomPage() {
  ...
  if (!room || !room.memberIds.includes(v.me.id)) return <div className="min-h-full"><TopBar back title="채팅" /><EmptyState emoji="🔒" title="입장할 수 없는 채팅방이에요" description="참가 승인이 완료되면 그룹 채팅방에 입장할 수 있어요." /></div>;
  ...
  const title = other?.nickname ?? room.title ?? '채팅';
  ...
  return (
    <div className="h-full flex flex-col">
      <TopBar back title={<button className="text-left" onClick={() => (other ? nav(`/users/${other.id}`) : activity ? nav(`/activities/${activity.id}`) : undefined)}><span className="block truncate">{title}</span>{room.type !== 'direct' && <span className="block text-[11px] text-ink-3 font-medium">멤버 {room.memberIds.length}명</span>}</button>}
        right={<button onClick={() => setMenu(true)} className="h-10 w-10 grid place-items-center rounded-full" aria-label="메뉴"><MoreVertical size={20} /></button>} />
      <div className="flex-1 overflow-y-auto hide-scrollbar px-4 py-3 space-y-2">
        ...
        {room.messages.length === 0 && <div className="text-center text-[12px] text-ink-3 py-8">대화를 시작해보세요. 차단·신고는 오른쪽 위 메뉴에서 할 수 있어요.</div>}
      ...
      <form className="shrink-0 border-t border-line bg-surface p-2.5 flex gap-2 safe-bottom" onSubmit={(e) => { e.preventDefault(); send(); }}>
        <input className="flex-1 h-11 rounded-2xl bg-surface-2 px-4 text-[14px] outline-none focus:bg-surface focus:ring-1 ring-primary" placeholder={blocked ? '차단된 사용자와는 대화할 수 없어요' : '메시지 입력…'} value={text} onChange={(e) => setText(e.target.value)} disabled={blocked} />
        <Button type="submit" className="h-11 w-11 !px-0 rounded-2xl" disabled={!text.trim() || blocked} aria-label="보내기"><Send size={18} /></Button>
      </form>

      <BottomSheet open={menu} onClose={() => setMenu(false)} title={title}>
        <div className="space-y-1">
          {room.type !== 'direct' && <div className="px-3 py-2 text-[12px] text-ink-3 flex items-center gap-1"><Users size={13} />멤버 {room.memberIds.map((id) => v.userById(id)?.nickname ?? (id === v.me.id ? '나' : '')).filter(Boolean).join(', ')}</div>}
          {other && <SheetItem icon={<Flag size={18} />} label="사용자 신고" danger onClick={() => { setMenu(false); setReport(true); }} />}
          {other && <SheetItem icon={<Ban size={18} />} label={blocked ? '차단 해제' : '사용자 차단'} danger onClick={() => { setMenu(false); blocked ? run(() => api.relationships.unblock(v.me.id, other.id), '차단을 해제했어요.') : setBlockOpen(true); }} />}
          {activity && activity.hostId !== v.me.id && <SheetItem icon={<LogOut size={18} />} label="활동 나가기" danger onClick={async () => { setMenu(false); await run(() => api.activities.cancel(activity.id, v.me.id), '활동에서 나갔어요.'); nav('/chats?tab=activity'); }} />}
          {!other && !activity && <SheetItem icon={<Flag size={18} />} label="채팅방 신고" danger onClick={() => { setMenu(false); setReport(true); }} />}
        </div>
      </BottomSheet>
      ...
      {other && (
        <Dialog open={blockOpen} onClose={() => setBlockOpen(false)} title={`${other.nickname}님을 차단할까요?`} description="차단하면 서로 메시지를 보낼 수 없고 프로필과 활동이 보이지 않아요.">
          <Button variant="outline" full onClick={() => setBlockOpen(false)}>취소</Button>
          <Button variant="danger" full onClick={async () => { setBlockOpen(false); await run(() => api.relationships.block(v.me.id, other.id), '차단했어요.'); nav('/chats'); }}>차단</Button>
        </Dialog>
      )}
    </div>
  );
