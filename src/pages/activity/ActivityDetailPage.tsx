...
export function ActivityDetailPage() {
  ...
  if (!a) return <div className="min-h-full"><TopBar back title="활동" /><EmptyState emoji="🔒" title="활동을 볼 수 없어요" description="삭제되었거나 공개 범위에 포함되지 않은 활동이에요." action={<Button onClick={() => nav('/')}>홈으로</Button>} /></div>;
  ...
  return (
    <div className="min-h-full pb-28">
      ...
      <div className="px-4 -mt-6 relative">
        ...
        <button onClick={() => nav(org ? `/orgs/${org.id}` : `/users/${a.hostId}`)} className="card mt-3 p-3 w-full flex items-center gap-3 text-left press">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[11px] text-ink-3">주최자</div>
        ...
        <div className="card mt-3 divide-y divide-line">
          <Row icon={<Clock size={16} />} label="날짜와 시간" value={`${formatDate(a.date)} ${formatTime(a.startTime)} – ${formatTime(a.endTime)}`} />
          <Row icon={<MapPin size={16} />} label="장소" value={a.place.name} sub={a.place.address} />
          <Row icon={<Users size={16} />} label="모집 인원" value={`${approved.length}명 참가 중 · 모집 ${a.capacity >= 999 ? '제한 없음' : `${a.capacity}명`}`} />
          <Row icon={<Ticket size={16} />} label="참가 비용" value={formatFee(a.fee)} />
          <Row icon={<Lock size={16} />} label="참가 방식" value={JOIN_POLICY_LABELS[a.joinPolicy]} />
          {a.conditions && <Row icon={<ShieldCheck size={16} />} label="참가 조건" value={a.conditions} />}
        </div>
        ...
        <div className="card mt-3 overflow-hidden">
          ...
          <button onClick={() => nav(`/map?focus=${a.id}`)} className="w-full h-11 text-[13px] font-semibold text-primary flex items-center justify-center gap-1"><MapPin size={14} />지도에서 보기</button>
        </div>

        <div className="card mt-3 p-4">
          <div className="flex items-center justify-between"><b className="text-[14px]">현재 참가자 {approved.length}</b>{isHost && <button onClick={() => nav(`/activities/${a.id}/manage`)} className="text-[12px] font-semibold text-primary">참가자 관리</button>}</div>
          <div className="flex flex-wrap gap-2 mt-3">
            {host && <PersonPill id={host.id} label="주최" />}
            {approved.map((p) => <PersonPill key={p.id} id={p.userId} />)}
            {approved.length === 0 && <span className="text-[12px] text-ink-3">아직 참가자가 없어요. 첫 참가자가 되어보세요!</span>}
          </div>
        </div>

        <div className="card mt-3 p-4">
          <b className="text-[14px]">댓글 또는 질문 {a.comments.length}</b>
          <div className="mt-3 space-y-3">
            {a.comments.map((c) => { const u = v.userById(c.authorId); return (
              <div key={c.id} className="flex gap-2.5"><Avatar emoji={u?.avatar.emoji ?? '👤'} hue={u?.avatar.hue ?? 200} size={30} /><div className="flex-1"><div className="text-[12px]"><b>{u?.nickname}</b>{c.authorId === a.hostId && <Tag tone="primary" className="ml-1 h-5">주최자</Tag>}<span className="text-ink-3 ml-1.5">{relativeTime(c.createdAt)}</span></div><p className="text-[13px] text-ink-2 mt-0.5">{c.text}</p></div></div>
            ); })}
            {a.comments.length === 0 && <p className="text-[12px] text-ink-3">궁금한 점을 주최자에게 물어보세요.</p>}
          </div>
          <form className="flex gap-2 mt-3" onSubmit={async (e) => { e.preventDefault(); if (!text.trim()) return; await run(() => api.activities.comment(a.id, v.me.id, text.trim())); setText(''); }}>
            <Input className="h-10" placeholder="질문이나 댓글 남기기" value={text} onChange={(e) => setText(e.target.value)} /><Button size="sm" className="h-10" type="submit" disabled={!text.trim()}>등록</Button>
          </form>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-surface/95 backdrop-blur border-t border-line p-3 flex gap-2 safe-bottom z-20">
        <button onClick={() => showToast('링크를 복사했어요.')} className="h-[52px] w-[52px] rounded-2xl bg-surface-2 grid place-items-center text-ink-2 press" aria-label="공유"><Share2 size={20} /></button>
        {canChat ? <Button size="lg" variant="secondary" onClick={() => nav(`/chats/${room!.id}`)} icon={<MessageCircle size={18} />}>그룹 채팅</Button> : null}
        <JoinButton activity={a} size="lg" className="flex-1" />
      </div>

      <BottomSheet open={menu} onClose={() => setMenu(false)} title="활동">
        <div className="space-y-1">
          {isHost && <SheetItem icon={<Pencil size={18} />} label="활동 수정" onClick={() => { setMenu(false); nav(`/activities/${a.id}/edit`); }} />}
          {isHost && <SheetItem icon={<Trash2 size={18} />} label="활동 삭제" danger onClick={() => { setMenu(false); setConfirmDelete(true); }} />}
          <SheetItem icon={<Share2 size={18} />} label="공유" onClick={() => { setMenu(false); showToast('링크를 복사했어요.'); }} />
          {!isHost && <SheetItem icon={<Flag size={18} />} label="활동 신고" danger onClick={() => { setMenu(false); setReport(true); }} />}
        </div>
      </BottomSheet>
      <ReportSheet open={report} onClose={() => setReport(false)} targetType="activity" targetId={a.id} />
      <Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)} title="활동을 삭제할까요?" description="참가자와 그룹 채팅방도 함께 사라져요. 되돌릴 수 없어요.">
        <Button variant="outline" full onClick={() => setConfirmDelete(false)}>취소</Button>
        <Button variant="danger" full onClick={async () => { await run(() => api.activities.remove(a.id), '활동을 삭제했어요.'); nav('/', { replace: true }); }}>삭제</Button>
      </Dialog>
    </div>
  );
}
