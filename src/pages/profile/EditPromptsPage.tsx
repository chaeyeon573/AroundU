...
export function EditPromptsPage() {
  ...
  const save = async () => {
    setBusy(true);
    try {
      await run(() => api.users.update(me.id, { prompts, voicePrompt: voice, poll, fieldVisibility: { ...me.fieldVisibility, prompts: visibility } }), '질문 답변을 저장했어요.');
  ...
  return (
    <div className="min-h-full pb-28">
      <TopBar back title="질문 답변" right={<VisibilityPicker compact value={visibility} onChange={setVisibility} label="질문 답변 공개 범위" />} />
      <div className="px-4 py-4 space-y-5">
        <div className="card p-4"><div className="text-[12px] font-semibold text-ink-3 mb-2">프로필 완성도</div><CompletionMeter {...preview} /></div>
        <Field label={`텍스트 질문 (${answered}/${REQUIRED_TEXT_PROMPTS} 필수, 최대 6개)`} required><PromptEditor value={prompts} onChange={setPrompts} /></Field>
        <Field label="음성 질문 (선택)"><VoicePromptEditor value={voice} onChange={setVoice} /></Field>
        <Field label="투표형 질문 (선택)" hint="방문자가 한 표씩 던질 수 있어요. 질문을 바꾸면 기존 투표는 사라져요."><PollEditor value={poll} onChange={setPoll} /></Field>
      </div>
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-surface/95 backdrop-blur border-t border-line p-3 safe-bottom z-20">
        <Button full size="lg" loading={busy} disabled={!ok} onClick={save}>{ok ? '저장' : `필수 질문 ${REQUIRED_TEXT_PROMPTS - Math.min(answered, REQUIRED_TEXT_PROMPTS)}개 더 답해주세요`}</Button>
      </div>
    </div>
  );
}
