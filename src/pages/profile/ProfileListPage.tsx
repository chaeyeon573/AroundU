...
const TITLES: Record<string, string> = { friends: '친구', followers: '팔로워', following: '팔로잉', likes: '관심 목록' };
...
export function ProfileListPage() {
  ...
  return (
    <div className="min-h-full">
      <TopBar back title={TITLES[list] ?? '목록'} />
      {list === 'likes' && <p className="px-4 pt-3 text-[12px] text-ink-3 flex items-center gap-1"><Heart size={12} className="text-heart" />관심은 상대에게 공개되지 않아요. 서로 관심을 표시하면 매칭돼요.</p>}
      <div className="px-4 py-3">
        {ids.length === 0 ? <EmptyState emoji="🙋" title="아직 아무도 없어요" /> : (
          <div className="card divide-y divide-line">
            {ids.map((id) => {
              const u = v.userById(id); const o = orgs.find((x) => x.id === id);
              if (o) return <button key={id} onClick={() => nav(`/orgs/${id}`)} className="w-full flex items-center gap-3 px-3.5 py-3 text-left press"><Avatar emoji={o.logo.emoji} hue={o.logo.hue} size={40} className="!rounded-xl" /><div className="flex-1"><b className="text-[14px]">{o.name}</b><div className="text-[12px] text-ink-3">조직</div></div></button>;
              if (!u) return null;
              return <button key={id} onClick={() => nav(`/users/${id}`)} className="w-full flex items-center gap-3 px-3.5 py-3 text-left press"><Avatar emoji={u.avatar.emoji} hue={u.avatar.hue} size={40} /><div className="flex-1 min-w-0"><b className="text-[14px] flex items-center gap-1">{u.nickname}{u.affiliation.type === 'university' && u.affiliation.emailVerified && <VerifiedBadge />}</b><div className="text-[12px] text-ink-3 truncate">{affiliationText(u)}</div></div>{list === 'likes' && v.isMutual(id) && <span className="text-[11px] font-bold text-heart">매칭</span>}</button>;
            })}
          </div>
        )}
      </div>
