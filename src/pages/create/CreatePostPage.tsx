...
export function CreatePostPage() {
  ...
  const submit = async () => {
    setBusy(true);
    try { await run(() => api.posts.create(v.me.id, form), '게시물을 올렸어요.'); nav('/community', { replace: true }); } catch { setBusy(false); }
  };

  return (
    <div className="min-h-full pb-28">
      <TopBar back title="게시물 작성" />
      <div className="px-4 py-4 space-y-5">
        <Field label="사진" hint="데모에서는 이모지 카드로 사진을 대체해요. 여러 장 추가할 수 있어요.">
        ...
        <Field label="내용" required><Textarea placeholder="무슨 일이 있었나요?" value={form.text} onChange={(e) => patch({ text: e.target.value })} maxLength={500} /></Field>
        <Field label="태그">
          <div className="flex gap-2"><Input placeholder="태그 입력 후 추가" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); if (tagInput.trim()) { patch({ tags: [...form.tags, tagInput.trim().replace(/^#/, '')] }); setTagInput(''); } } }} /><Button variant="secondary" onClick={() => { if (tagInput.trim()) { patch({ tags: [...form.tags, tagInput.trim().replace(/^#/, '')] }); setTagInput(''); } }}>추가</Button></div>
        ...
        <Field label="관련 활동 (선택)" hint="연결하면 게시물에 '이 활동에 참여하기' 버튼이 표시돼요.">
          <Select value={form.relatedActivityId ?? ''} onChange={(e) => patch({ relatedActivityId: e.target.value || undefined })}><option value="">연결 안 함</option>{myActs.map((a) => <option key={a.id} value={a.id}>{a.title}</option>)}</Select>
        </Field>
        {myOrgs.length > 0 && <Field label="조직 이름으로 게시"><Select value={form.orgId ?? ''} onChange={(e) => patch({ orgId: e.target.value || undefined })}><option value="">내 이름으로</option>{myOrgs.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</Select></Field>}
        <Field label="공개 범위" required><VisibilityList value={form.visibility} options={['public', 'school', 'friends']} onChange={(vis) => patch({ visibility: vis })} /></Field>
      </div>
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-surface/95 backdrop-blur border-t border-line p-3 safe-bottom z-20">
        <Button full size="lg" disabled={!form.text.trim()} loading={busy} onClick={submit}>게시하기</Button>
      </div>
    </div>
  );
}
