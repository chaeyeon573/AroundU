...
import { Button, Field, Input, Textarea, Select, VisibilityList, Cover, Toggle } from '@/components/ui';
...
export function CreatePostPage() {
  ...
  const [form, setForm] = useState<PostInput>({ text: '', media: [], tags: [], visibility: 'school', relatedActivityId: undefined, orgId: undefined, anonymous: false });
  ...
  return (
    <div className="min-h-full pb-28">
      ...
      <div className="px-4 py-4 space-y-5">
        <div className="card px-4"><Toggle label={t('익명으로 올리기')} description={t('이름과 사진 대신 "익명"으로 표시돼요. 학교명만 보여요. 신고 시에는 운영진이 확인할 수 있어요.')} checked={!!form.anonymous} onChange={(val) => patch({ anonymous: val, orgId: val ? undefined : form.orgId })} /></div>
        <Field label={t('사진')} hint={t('데모에서는 이모지 카드로 사진을 대체해요. 여러 장 추가할 수 있어요.')}>
          ...
          {form.media.length > 0 && <div className="flex flex-wrap gap-1.5 mt-2">{EMOJIS.map((e) => <button key={e} onClick={() => patch({ media: form.media.map((m, i) => (i === form.media.length - 1 ? { ...m, emoji: e } : m)) })} className={cn('h-9 w-9 rounded-lg text-lg', form.media[form.media.length - 1]?.emoji === e ? 'bg-primary-soft ring-2 ring-primary' : 'bg-surface-2')}>{e}</button>)}</div>}
        </Field>
        ...
        {myOrgs.length > 0 && !form.anonymous && <Field label={t('조직 이름으로 게시')}><Select value={form.orgId ?? ''} onChange={(e) => patch({ orgId: e.target.value || undefined })}><option value="">{t('내 이름으로')}</option>{myOrgs.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}</Select></Field>}
