...
export function Avatar({ emoji, hue, size = 44, className, ring }: AvatarProps) {
  return (
    <div
...
/** 큰 프로필 사진 (카드용) */
export function Portrait({ emoji, hue, className, photoType }: { emoji: string; hue: number; className?: string; photoType?: User['avatar']['photoType'] }) {
  return (
    <div
...
export function Cover({ emoji, hue, className, size = 40 }: { emoji: string; hue: number; className?: string; size?: number }) {
  return (
    <div
      className={cn('grid place-items-center select-none overflow-hidden', className)}
      style={{ background: `linear-gradient(135deg, hsl(${hue} 80% 92%) 0%, hsl(${(hue + 30) % 360} 75% 78%) 60%, hsl(${(hue + 50) % 360} 70% 68%) 100%)` }}
    >
      <span style={{ fontSize: size, lineHeight: 1 }} className="drop-shadow-sm">{emoji}</span>
    </div>
  );
}
